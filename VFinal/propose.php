...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Récupérer les données du formulaire de proposition de trajet
    $depart = $_POST['depart'];
    ...
    $requete = $mysqli->prepare("INSERT INTO Trajet (ID_conducteur, point_depart, point_arrivee, date, heure, NbPlace) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$requete) {
        die('Erreur de préparation de la requête : ' . $mysqli->error);
    }
    $bind = $requete->bind_param("issssi", $id_conducteur, $depart, $destination, $date, $heure, $places);

    if (!$bind) {
        die('Erreur lors de la liaison des paramètres : ' . $requete->error);
    }
    
    $execute = $requete->execute();
    
    if($execute){
        // La proposition de trajet a réussi, rediriger l'utilisateur vers une page de confirmation
        header("Location: recherche.php");
        exit;
    } else {
        // Afficher un message d'erreur en cas d'échec de la proposition de trajet
        header("Location: propose.php?error=Erreur lors de la proposition de trajet");
        exit;
    }
...
<body>
	...
	<!-- Banniere avec les vehicules -->
	
	<div class="banniere">
	<div class="road">
		<img src="image/car.png" alt="Voiture" class="car top-car">
		<img src="image/car-sport.png" alt="Voiture" class="car bottom-car">
    </div>
	</div>
    ...
    <form id="propose-form" action="propose.php" method="POST">
        <h1>Proposer un Trajet</h1><br><br>
	...
	<footer>
		...
		<div class="logo-container">
			<a href="recherche.html"><img src="image/CovoitUPHF2.png" alt="Logo"></a>
		</div>
